...
from ansible_base.rbac.triggers import (
    dab_post_migrate,
    defer_rbac_computations,
    post_migration_rbac_setup,
)
...
@pytest.mark.django_db
@override_settings(ANSIBLE_BASE_ALLOW_CUSTOM_TEAM_ROLES=True)
def test_defer_rbac_computations_delete_org_cleans_team_assignments(rando):
    """Deleting an org within defer_rbac_computations must also remove
    RoleUserAssignment and RoleTeamAssignment records for cascade-deleted teams,
    not just ObjectRoles and RoleEvaluations."""
    from test_app.models import Organization, Team

    org = Organization.objects.create(name="defer-org-assign-cleanup")
    team = Team.objects.create(name="defer-team-assign-cleanup", organization=org)
    team_ct = permission_registry.content_type_model.objects.get_for_model(Team)

    team_member_rd = RoleDefinition.objects.create_from_permissions(
        permissions=["view_team", "member_team"],
        name="team-member-assign-cleanup",
        content_type=team_ct,
    )
    assignment = team_member_rd.give_permission(rando, team)
    assert RoleUserAssignment.objects.filter(object_id=team.pk, content_type=team_ct).exists()
...
def test_defer_rbac_error_handling_paths(organization, rando, org_inv_rd, scenario):
    ...
    if scenario == "rollback":
        ...
        with patch(
            "ansible_base.rbac.triggers._flush_rbac",
            side_effect=RuntimeError("flush error"),
        ):
            with pytest.raises(RuntimeError, match="deliberate"):
                _create_and_raise()
...
def test_defer_rbac_computations_team_creation():
    ...
    with patch(
        ...
        wraps=compute_team_member_roles,
    ) as mock_ctmr:
        with defer_rbac_computations():
