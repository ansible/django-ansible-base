...
def test_inventory_creator_options(user, user_api_client, organization, org_inv_admin, view_inv_rd):
...
@pytest.mark.django_db
def test_organization_creator_options(user, user_api_client, admin_api_client, view_inv_rd):
...
def test_object_change_permission(user, user_api_client, inventory, inv_rd, view_inv_rd):
    ...
    assert 'PATCH' in r.data['actions']
    assert 'PUT' in r.data['actions']


@pytest.mark.django_db
def test_object_delete_permission(user, user_api_client, inventory, inv_rd, view_inv_rd):
    url = reverse('inventory-detail', kwargs={'pk': inventory.pk})
    view_inv_rd.give_permission(user, inventory)
    assert user.has_obj_perm(inventory, 'delete') is False

    r = user_api_client.options(url)
    assert r.status_code == 200
    assert 'actions' not in r.data  # no DELETE

    inv_del = RoleDefinition.objects.create_from_permissions(
        name='org-inv-admin',
        permissions=['add_inventory', 'view_organization'],
        content_type=permission_registry.content_type_model.objects.get_for_model(Organization)
    )
