...
def test_association_router_basic_viewset():
    ...
    validate_expected_url_pattern_names(
        router, ["organization-list", "organization-detail"]
    )
...
def test_association_router_good_associate(
    db, admin_api_client, randname, organization
):
    ...
    url = get_relative_url(
        "related_fields_test_model-more_teams-associate",
        kwargs={"pk": related_model.pk},
    )
...
def test_association_router_associate_bad_data(
    db, admin_api_client, data, response_instances
):
    related_model = RelatedFieldsTestModel.objects.create()
    assert related_model.users.count() == 0

    url = get_relative_url(
        "related_fields_test_model-users-associate", kwargs={"pk": related_model.pk}
    )
...
def test_association_router_associate_existing_item(db, admin_api_client, random_user):
    ...
    response = admin_api_client.post(
        url, data={"instances": [random_user.pk]}, format="json"
    )
    assert response.status_code == 204
...
def test_association_router_disassociate(db, admin_api_client, randname, organization):
    ...
    url = get_relative_url(
        "related_fields_test_model-more_teams-disassociate",
        kwargs={"pk": related_model.pk},
    )
    response = admin_api_client.post(url, data={"instances": [team.pk]})
    assert response.status_code == 204
...
@pytest.mark.parametrize(
    ...
    [
        (
            {"instances": [-1]},
            [
                'Invalid pk "-1" - object does not exist or is not associated with parent object.'
            ],
        ),
        ({"instances": ["a"]}, ["Incorrect type. Expected pk value, received str."]),
        ({"instances": [True]}, ["Incorrect type. Expected pk value, received bool."]),
...
def test_association_router_disassociate_bad_data(
    db, admin_api_client, data, response_instances
):
    related_model = RelatedFieldsTestModel.objects.create()
    assert related_model.more_teams.count() == 0
...
def test_association_router_disassociate_something_not_associated(
    db, admin_api_client, organization
):
    related_model = RelatedFieldsTestModel.objects.create()
    ...
    response = admin_api_client.post(
        url, data={"instances": [team1.pk, team2.pk, team3.pk]}, format="json"
    )
    assert response.status_code == 400
    assert response.json().get("instances") == [
        f'Invalid pk "{team2.pk}" - object does not exist or is not associated with parent object.'
    ]
...
def test_sublist_filtering(organization, admin_api_client):
    ...
    url = get_relative_url(
        "organization-inventories-list", kwargs={"pk": organization.pk}
    )
...
def test_sublist_override_filtering(
    organization, inventory, user_api_client, user, org_member_rd
):
    ...
    inventory_url = get_relative_url(
        "organization-inventories-list", kwargs={"pk": organization.pk}
    )
...
def test_associated_view_description(admin_api_client, organization):
    ...
    assert response.status_code == 200, response.data
    assert (
        "GET /:id/cows/ to show cows currently in the relationship"
        in response.data.get("description", "")
    )
...
def test_associate_disassociate_schema_returns_204(admin_api_client, endpoint_suffix):
    """Verify that associate/disassociate endpoints declare 204 (not 200) in the OpenAPI schema.

    Regression test for AAP-57081: the auto-generated OpenAPI spec incorrectly
    declared a 200 response with a serializer body for these endpoints, even
    though the implementation returns HTTP 204 No Content with no body.
    """
    ...
    # Find any path that ends with /{endpoint_suffix}/
    matching_paths = {
        path: ops
        for path, ops in schema["paths"].items()
        if path.rstrip("/").endswith(f"/{endpoint_suffix}")
    }
    assert matching_paths, f"No paths ending with /{endpoint_suffix}/ found in schema"

    for path, operations in matching_paths.items():
        post_op = operations.get("post")
        assert post_op is not None, f"POST operation missing for {path}"

        responses = post_op.get("responses", {})
        assert "204" in responses, (
            f"{path} should declare a 204 response, but only has: {list(responses.keys())}"
        )
        assert "200" not in responses, (
            f"{path} should NOT declare a 200 response (actual return is 204 No Content)"
        )
