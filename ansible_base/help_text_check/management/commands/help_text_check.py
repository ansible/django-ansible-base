    ...
    def handle(self, *args, **options):
        ...
        if ignore_file:
            try:
                with open(ignore_file, 'r') as f:
                    self.ignore_fields = [line.rstrip() for line in f]
            except FileNotFoundError:
                self.stderr.write(f"Ignore file {ignore_file} does not exist")
                exit(255)
            except PermissionError:
                self.stderr.write(f"No permission to read {ignore_file}")
                exit(255)
            except Exception as e:
                self.stderr.write(f"Failed to read {ignore_file}: {e}")
                exit(255)

            if len(self.ignore_fields) > 0:
                self.stdout.write(f"Ignoring {len(self.ignore_fields)} fields:")
        ...
        for model in models:
            ...
            for field in model._meta.concrete_fields:
                ...
                if field_name in self.ignore_fields or (field.name in self.global_ignore_fields and not options.get('skip_global_ignore', False)):
                    message = self.style.WARNING(f"{self.indentation}{field.name}: ignoring")
                elif not help_text:
                    return_code = 1
