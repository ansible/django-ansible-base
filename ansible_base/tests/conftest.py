...
def ldap_authenticator(ldap_configuration):
    ...
    authenticator = Authenticator.objects.create(
        name="Test LDAP Authenticator", enabled=True, create_objects=True, users_unique=False, remove_users=True, type="ansible_base.authenticator_plugins.ldap", configuration=ldap_configuration
    )
    yield authenticator
...
def local_authenticator(db):
    ...
    authenticator = Authenticator.objects.create(
        name="Test Local Authenticator", enabled=True, create_objects=True, users_unique=False, remove_users=True, type="ansible_base.authenticator_plugins.local", configuration={}
    )
    yield authenticator
...
def keycloak_authenticator(db):
    ...
    authenticator = Authenticator.objects.create(
        name="Test Keycloak Authenticator", enabled=True, create_objects=True, users_unique=False, remove_users=True, type="ansible_base.authenticator_plugins.keycloak", configuration={}
    )
    yield authenticator
    authenticator.delete()
