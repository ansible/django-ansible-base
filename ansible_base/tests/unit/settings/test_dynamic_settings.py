...
def test_authentication_with_backends():
    additional_config = dedent(
        ...
        ANSIBLE_BASE_FEATURES = {'AUTHENTICATION': True}
        REST_FRAMEWORK = {}
    '''
    )
    ...
    assert 'ansible_base.utils.middleware.AuthenticatorBackendMiddleware' in updated_settings['MIDDLEWARE']
    ...
    assert 'ansible_base.authenticator_plugins' in updated_settings['ANSIBLE_BASE_AUTHENTICATOR_CLASS_PREFIXES']
...
def test_append_middleware():
    ...
    updated_settings = get_updated_settings(additional_config)
    assert 'ansible_base.utils.middleware.AuthenticatorBackendMiddleware' == updated_settings['MIDDLEWARE'][-1]
