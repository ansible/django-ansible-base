...
class TokenView(oauth_views.TokenView):
    def create_token_response(self, request):
        ...
        if request.POST.get('grant_type') == 'refresh_token' and 'refresh_token' in request.POST:
            refresh_token = OAuth2AccessRefreshToken.objects.filter(token=request.POST['refresh_token']).first()
            if refresh_token:
...
class ApiOAuthAuthorizationRootView(APIView):
    ...
    def get(self, request, format=None):
        data = OrderedDict()
        data['authorize'] = _reverse('api:authorize')
        data['token'] = _reverse('api:token')
        data['revoke_token'] = _reverse('api:revoke-token')
