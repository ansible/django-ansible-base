    ...
    def flushdb(self, asynchronous: Optional[bool] = None, **kwargs) -> ResponseT:
        ...
        if self.__class__ == DABRedisCluster:
            ...
            raise ImproperlyConfigured(
                f"Got an inappropriate type of client {self.__class__}"
            )
    ...
    def _redis_parse_url(self) -> None:
        ...
        if parsed_url.scheme in ["file", "unix"]:
            ...
            if (
                ...
                and self.connection_settings.get("ssl", None) is None
            ):
            ...
            if self.connection_settings.get("ssl") and parsed_url.scheme != "rediss":
                ...
                logger.info(
                    "Altering redis URL to be rediss because tls was specified in kwargs"
                )
        else:
            raise ImproperlyConfigured(
                _(
                    "Invalid redis URL, can only have the scheme file, unix, redis or rediss URLs, got {}"
                ).format(self.url)
            )
    ...
    def _get_hosts(self) -> None:
        ...
        try:
            if not isinstance(self.redis_hosts, str):
                logger.error(
                    f"Specified redis_hosts is not a string, got: {self.redis_hosts}"
                )
            ...
            for host_port in host_ports:
                ...
                if not response.port:
                    logger.error(
                        f"Specified cluster_host {host_port} is not valid; it needs to be in the format <host>:<port>"
                    )
                    had_host_errors = True
                    continue
                ...
                self.connection_settings["startup_nodes"].append(
                    ClusterNode(response.address, port)
                )
    ...
    def get_client(self, url: str = "", **kwargs) -> Union[DABRedis, DABRedisCluster]:
        ...
        for file_setting in ["ssl_certfile", "ssl_keyfile", "ssl_ca_certs"]:
            ...
            if file == "":
                ...
                file is not None
                and self.connection_settings.get("ssl", None)
                and not os.access(file, os.R_OK)
            ):
                raise ImproperlyConfigured(
                    _("Unable to read file {} from setting {}").format(
                        file, file_setting
                    )
                )
        ...
        if self.mode == "cluster":
            ...
            for setting in _INVALID_STANDALONE_OPTIONS:
                if setting in self.connection_settings:
                    logger.info(
                        f"Removing setting {setting} from connection settings because its invalid for standalone mode"
                    )
            ...
            raise ImproperlyConfigured(
                _("mode must be either one of ['cluster', 'standalone'] got {}").format(
                    self.mode
                )
            )
...
def get_redis_status(
    url: str = "", timeout: int = _DEFAULT_STATUS_TIMEOUT_SEC, **kwargs
) -> Dict:
    ...
    try:
        ...
        if redis_client.mode == "standalone":
            ...
            response["status"] = determine_cluster_node_status(
                response["cluster_nodes"]
            )
...
def determine_cluster_node_status(cluster_nodes: Dict) -> str:
    # Build master-replica topology and check for unrecoverable failures.
    # A failed master with no healthy replicas means hash slots are permanently
    # lost, which is a FAILED state (not merely DEGRADED).
    has_bad_node = False

    # Identify masters and build a map of master_id -> list of replica nodes
    masters = {}  # node_id -> node dict
    replicas_by_master = {}  # master node_id -> list of replica node dicts

    for response_node in cluster_nodes.values():
        if "fail" in response_node["flags"]:
            has_bad_node = True

        master_id = response_node.get("master_id")
        node_id = response_node.get("node_id")

        if master_id == "-":
            # This is a master node
            masters[node_id] = response_node
            if node_id not in replicas_by_master:
                replicas_by_master[node_id] = []
        else:
            # This is a replica node; track it under its master
            if master_id is not None:
                replicas_by_master.setdefault(master_id, [])
                replicas_by_master[master_id].append(response_node)

    if not has_bad_node:
        return STATUS_GOOD

    # Check each failed master: does it have at least one healthy replica?
    for master_id, master_node in masters.items():
        if "fail" not in master_node["flags"]:
            continue
        # This master is failed -- check its replicas
        replicas = replicas_by_master.get(master_id, [])
        has_healthy_replica = any("fail" not in r["flags"] for r in replicas)
        if not has_healthy_replica:
            # No healthy replica can take over -> hash slots are lost
